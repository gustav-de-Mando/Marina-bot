...
class RolesTags(commands.Cog):
    ...
    async def addrank(self,ctx,role:discord.Role):execute('INSERT OR REPLACE INTO joinable_ranks(guild_id,role_id,name) VALUES(?,?,?)',(ctx.guild.id,role.id,role.name.lower()));await ctx.send(f'✅ {role.mention} ist jetzt ein Rank.')
    @commands.hybrid_command(name='delrank',description='Entfernt einen beitretbaren Rank.')
    @commands.has_permissions(manage_roles=True)
    async def delrank(self,ctx,role:discord.Role):execute('DELETE FROM joinable_ranks WHERE guild_id=? AND role_id=?',(ctx.guild.id,role.id));await ctx.send('✅ Rank entfernt.')
    @commands.hybrid_command(name='rank',description='Tritt einem beitretbaren Rank bei oder verlässt ihn.')
    async def rank(self,ctx,role:discord.Role):
        if not one('SELECT 1 FROM joinable_ranks WHERE guild_id=? AND role_id=?',(ctx.guild.id,role.id)):return await ctx.send('❌ Diese Rolle ist kein beitretbarer Rank.')
    ...
    @commands.hybrid_command(name='ranks',description='Listet alle beitretbaren Ranks.')
    async def ranks(self,ctx):
        rs=query('SELECT role_id FROM joinable_ranks WHERE guild_id=?',(ctx.guild.id,));roles=[ctx.guild.get_role(x['role_id']) for x in rs];roles=[r for r in roles if r];await ctx.send('**Ranks:** '+(', '.join(r.mention for r in roles) if roles else 'Keine'))
    ...
    @commands.hybrid_command(name='roles',description='Listet Serverrollen.')
    async def roles(self,ctx):await ctx.send((' '.join(r.mention for r in reversed(ctx.guild.roles[1:])) or 'Keine Rollen.')[:1900])
    @commands.hybrid_command(name='tag',description='Tag anzeigen oder mit create/edit/delete verwalten.')
    ...
    @commands.hybrid_command(name='tags',description='Listet Tags.')
